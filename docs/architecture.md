...
- **v4 (this version):** Adds §4.13, generalizing §4.12 Principle 3 beyond localization specifically — a domain object should never carry the words used to describe it to a player, only the fact those words describe. Motivated by M11 design work on dungeon-entry preparedness assessments (`game-design.md` §1, §8). Considered and rejected as a second ADR: it's a scope clarification of a principle ADR-0001 already established, with no new rejected alternative or open mechanism decision to record, so it's written here as guidance rather than as ADR-0002.
