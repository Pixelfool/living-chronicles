...
- **v2:** Incorporates a critical self-review. Four issues turned out to be real bugs-waiting-to-happen, not just style preferences, and got structural fixes: cross-module transactional consistency (§4.4), event bus failure isolation (§4.7), module-boundary enforcement being weaker than implied (§4.4), and background-job process isolation (§2, §9). Several other decisions turned out to be real, load-bearing limitations that v1 described too optimistically — those are now stated plainly rather than glossed over (plugin hot-install, auth model vs. multi-client goals, SPA vs. SEO/accessibility). Keeping this changelog is itself the discipline the ADR process asks for — don't delete it in v3, add to it.
- **v3:** No new design decisions — reconciles §4.4 with implementation experience across every milestone that's touched cross-module writes so far (Combat/World onward through M7-M9). The "orchestrating use case" framing in §4.4 described a shape (a dedicated class per operation, calling other modules' tx-accepting methods) that six independent implementations never actually used; §4.4 now also records the conditional-claim idiom those same implementations converged on for single-table races, which was never written down at all. §4.7 gets one line noting the event-payload-typing convention every existing emit call already follows. A retrospective on already-shipped code, not a new decision.
- **v4:** Adds §4.13, generalizing §4.12 Principle 3 beyond localization specifically — a domain object should never carry the words used to describe it to a player, only the fact those words describe. Motivated by M11 design work on dungeon-entry preparedness assessments (`game-design.md` §1, §8). Considered and rejected as a second ADR: it's a scope clarification of a principle ADR-0001 already established, with no new rejected alternative or open mechanism decision to record, so it's written here as guidance rather than as ADR-0002.
- **v5 (this version):** Adds §4.14, naming a risk surfaced during M12 (World Events) code review but deliberately not solved there: persisted state that references a content value *permanently* (World Events' `resolvedOutcome`) can outlive that content value if it's later edited. No mechanism chosen — recorded as a named, revisit-triggered risk (consistent with this document's general discipline of naming a real problem before it's solved), not a new subsystem.
...
| Localization | **Architectural principles only, for now** (§4.12) — no message concatenation, business logic never depends on localized text, plugins own their own translations; no library, loader, or resolution mechanism chosen yet | Committing to i18next / nestjs-i18n / a custom service today — premature while Combat, Quests, NPC dialogue, and plugin content haven't defined what localization actually needs to support |
...
**Confirmed in practice (Combat, World, Crafting, Economy, Quests):** the "thin orchestration layer" above is not a separate use-case class per operation — it's the initiating module's own service method. Combat's `fight()`, World's `travel()`, Crafting's `startCraft()`/job resolution, Economy's shop `buy()`/`sell()` and trade `accept()`, and Quests' `complete()` have all independently converged on the same shape: the module that owns the triggering action opens its own `$transaction` and reaches directly into whichever other modules' Prisma models it needs (`tx.character`, `tx.itemInstance`), rather than calling another module's service with a passed-in `tx`. No module exposes a tx-accepting method for this purpose — the only one in the codebase is `AuditLogService.record(tx, ...)`, and it only ever writes an audit row, never proxies a domain mutation. Every one of these transactions also emits its events strictly after commit, never before, consistent with item 4 above. This is the actual v1 shape of the pattern; the `PurchaseItemUseCase`-style framing above is still the honest target *if* this is ever pulled into shared, reusable infrastructure, but six independent implementations later, nothing has needed that yet.

**The conditional-claim idiom (confirmed in practice, same modules as above, plus Character and Guilds):** the mechanism that makes each of those transactions race-safe on its own contested fields is a specific, repeated pattern — claim with a conditional `updateMany`/`deleteMany` whose `where` clause encodes the precondition (`actionPoints: { gte: cost }`, `status: 'IN_PROGRESS'`, enough matching `itemInstance` rows), then check the returned count. Zero means someone else won the race, and the caller throws a `ConflictException` instead of proceeding on stale data. This is how action-point claims, gold and item claims, job/invite/offer state transitions, and quest completion all avoid lost updates without table-level locking or a plain read-then-write. The one case it doesn't cover so far is Quests' objective-progress update, a read-modify-write on an array field rather than a check against a scalar precondition — that one instance used a raw `SELECT ... FOR UPDATE` instead.
...
**Confirmed in practice:** every domain event emitted anywhere in the codebase is type-checked at the call site with `satisfies <EventName>Event` against its declared interface — no bare object-literal emits exist. That's what has kept an event's actual payload shape aligned with its declared interface so far; the one gap found during the M9 review was exactly a missing `satisfies` annotation.
...
### 4.12 Localization / i18n — Principles Now, Mechanism Later

*Recorded formally as `docs/adr/0001-localization-principles.md`, including rejected alternatives and the revisit trigger — this section is the summary.*

**Status:** this section defines binding *architectural principles* for how localization fits into the system — not a translation library, loader, or runtime resolution mechanism. That choice depends on requirements that don't exist yet (Combat's floating combat text and status effects, Quests' branching dialogue, NPC flavor text, plugin-provided content packs, background jobs and future non-HTTP APIs that may need to produce player-facing text with no request in scope at all). Committing to a mechanism now, before any of those actually exist, risks building infrastructure shaped by a guess instead of a real requirement — exactly the mistake §1's "don't build ahead of need" and §5.2's plugin-loader honesty already warn against elsewhere in this document. A principle below constrains what the architecture must always guarantee, regardless of how localization is eventually implemented; anything describing *how* it will work has been deliberately left undecided rather than written down here.

**Principle 1 — no user-facing message is assembled by concatenation.** A message with a dynamic value (a count, a name, a price) is a complete template with named placeholders, never built by joining translated fragments around a raw value in code (`'only ' + count + ' left'` is exactly what this forbids). This is the one principle that can't be retrofitted cheaply later — concatenation bakes in English word order and grammar that a language like German (different word order, case-dependent noun forms) often can't just slot a value into, no matter what mechanism eventually does the translating.

**Principle 2 — one message, one place.** A given user-facing message is defined once per module and referenced, not re-typed with slightly different wording every time the same situation occurs. This isn't new ceremony — it's the same discipline as not duplicating a magic number — but it's what makes a later mechanical extraction (message → key) a find-and-replace instead of an audit.

**Principle 3 — business logic never depends on localized text.** A service, guard, or event handler makes every decision on domain objects, typed error/result codes, event names, or IDs — never by inspecting, comparing, or branching on a translated (or even a default-locale) display string. Localized text is presentation, produced *from* a decision the domain has already made, for reasons entirely its own; it is never an input to one. Concretely: the string currently passed to a `throw` or an `emit` is a label on a decision the domain made for other reasons (not-found, conflict, a specific validation failure) — nothing in the domain may ever come to depend on that label being any particular value, translated or not.

**Principle 4 — plugins own their own localization resources.** A plugin or content pack that introduces its own user-facing text is responsible for its own translations, independently of core. Core is never a bottleneck for a plugin's language coverage, and a plugin's translations never need to live inside, or be merged into, core's own resource files. This is the same self-containment §4.9 and §5.1 already require of content packs generally — localization data is just one more thing a plugin brings with it rather than requesting from core.

**Deliberately left open, until real requirements exist:**
- Which library or custom mechanism eventually resolves a message to display text (i18next, nestjs-i18n, a small custom loader, or something else) — genuinely undecided, see the rejected alternative in §1's table.
- How locale is determined and threaded through — for an HTTP request, a WebSocket connection, a background job, or a plugin-triggered event with no request in scope at all. The mechanism (explicit parameter passing, request-scoped context, something else entirely) is intentionally not decided here — Combat, Quests, background jobs, and future APIs will each have their own shape of "what context is even available," and that's exactly the kind of detail this document shouldn't guess at yet.
...
- The concrete mechanics of "a plugin owns its translations" (a directory core discovers, a self-registered resource, something else) — Principle 4 fixes the self-containment requirement, not the wiring.

**What changes immediately:** nothing is retrofitted — the ~80 existing hardcoded exception messages across Combat, Guilds, Economy, Social, etc. stay exactly as they are (`build-plan-v1.md` §4 tracks converting them as each module is next touched for a real feature reason, not as a standalone pass). What changes is discipline going forward: **new** user-facing strings, in new or actively-touched modules, follow Principles 1–3 above from the moment they're written — plain, complete English messages today, organized so that giving them a translation key later is mechanical, not a rewrite.

### 4.13 Domain/Presentation Separation for Derived Player-Facing Judgments

Principle 3 above says business logic never depends on localized text — the string passed to a `throw` or an `emit` is a label on a decision the domain already made for its own reasons. M11 design work on dungeon-entry preparedness (`game-design.md` §1, §8) surfaced a case where the same boundary matters even with zero translation in scope: when a system needs to give a player a *judgment*, not a raw fact — how prepared they are for something, how risky a choice looks — the domain's job ends at producing a small, closed, discrete result (a named tier, not a sentence, and not a raw score handed to the client). Turning that result into what the player actually reads is content's job, resolved the same way `ContentService` already turns ids into display text elsewhere in the codebase (M9's `describeObjective`, `findMonster`/`findItem` lookups) — never something the domain composes itself.

The first real consumer is the dungeon-entry preparedness assessment (M11): a pure, deterministic function of character/dungeon facts producing a closed tier, with all wording owned by dungeon content. This is a reusable *shape*, not reusable *infrastructure* — implement it once, where M11 needs it, and don't extract a shared service until a second real consumer (a dangerous region, a PvP challenge, a hard quest) actually exists, per `build-plan-v1.md` §6's guardrail against generalizing a pattern used only once.

### 4.14 Content-Referenced Persisted State Can Outlive the Content It References

Surfaced during the M12 (World Events) code review, not resolved there: `WorldEventInstance.resolvedOutcome` is deliberately a plain content-owned string, not a Prisma enum (§4.6's `Character.profession` reasoning — a new event's outcome vocabulary must never require a migration). That's the right call for authoring flexibility, but it exposes a case this document hadn't named yet: a database row can persist a reference to a content value (an outcome tag, in this case) *indefinitely* — residue is explicitly meant to be permanent, per `game-design.md`'s M12 design discussion — while the content pack that defined that value is free to change on every deploy. If a later content edit renames or removes the tag a resolved instance still points to, any code that resolves that reference (here, `WorldEventsService.getForCity()` matching `resolvedOutcome` against `definition.outcomes`) has no fallback and no migration path — it just breaks, silently, for however long the mismatch goes unnoticed.

This is a general shape, not a World-Events-specific bug: anywhere persisted state is expected to reference content *forever* (as opposed to `DungeonRun`/`CraftingJob`, whose in-progress state is short-lived enough that a content edit invalidating it mid-flight is an acceptable, rare edge case), a content id going away out from under it is a real, if infrequent, failure mode. No mechanism is chosen here — a content-value deprecation convention, a fallback display string, or simply "don't ever remove an outcome tag, only add" are all plausible and none has been tried yet. Recording the shape of the problem now, so the next persisted-content-reference case doesn't have to rediscover it, without guessing at a mechanism ahead of a second real instance of the problem.

**Trigger to revisit:** a content edit actually breaks a persisted reference in practice (in World Events or elsewhere), or a second long-lived persisted-content-reference case makes the shared shape undeniable rather than theoretical.
