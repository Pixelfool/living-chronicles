...
| Shared fight-resolution helper (claim AP → `resolveFight` → apply HP/XP/loot → emit Battle/LevelUp/ItemAcquired) | — | Identified in the M12 code review: this exact block is now duplicated four times (`CombatService.fight`, `WorldService.travel`, `DungeonsService.resolveAndApplyBeat`, `WorldEventsService.fight`). Each copy was individually justified when written (build-plan-v1.md §1's "apply the pattern case-by-case" discipline), but four independent occurrences is real, proven repetition, not a hypothetical one — extracting now, mid-feature, would have mixed a refactor into M12's diff | A dedicated cleanup pass, done deliberately rather than folded into the next feature that happens to need a fight — or the next bug found in one copy that turns out to already be fixed in another |
| World event instance uniqueness per city (DB constraint or app-level check preventing two simultaneous ACTIVE/EMERGING instances for the same city) | `architecture.md` §4.14-adjacent (M12 review) | No spawn-selection mechanism exists yet to ever create a second instance for one city — the risk is real but currently unreachable | Designing the spawn-selection mechanism itself (already a separately deferred M12 item) — build the safeguard alongside it, not before it exists to protect against |
| Generalized per-response-type config shape for World Events (a keyed `responseConfig` instead of one bespoke optional field + `.refine()` per response type) | `content/schemas.ts`'s `WorldEventDefinitionSchema` | Two response types (FIGHT, SUPPORT) don't yet prove the current one-field-per-verb shape is actually a problem — generalizing now would be designing for a third verb that doesn't exist | A third response type (e.g. RESEARCH, for a plague-shaped event) is actually being added |
